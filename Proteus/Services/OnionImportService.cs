...
public sealed class OnionImportService
{
    ...
    internal static ImportPreview BuildPreview(string ompPath, OnionPackage.Contents pack, string? preferredBodyType)
    {
        ...
        if (m.FormatVersion > OnionPackage.KnownFormatVersion)
            warnings.Add($"The pack declares format version {m.FormatVersion}; Proteus has only been " +
                         $"checked against {OnionPackage.KnownFormatVersion}. Import it and check the result.");

        if (m.Groups is { ValueKind: JsonValueKind.Array } g && g.GetArrayLength() > 0)
            warnings.Add($"The pack has {g.GetArrayLength()} Onion option group(s). Those aren't imported — " +
                         "every layer is brought in unconditionally.");
        ...
        if (layers.Any(l => l.GeneratedFrom != null))
            warnings.Add("Some layouts were generated by Onion from another layout rather than painted. " +
        ...
        if (m.Layers != null && m.Layers.Any(l => l.Races is { ValueKind: JsonValueKind.Array } r && r.GetArrayLength() > 0))
            warnings.Add("Some layers are restricted to specific races. Proteus doesn't carry race filters, " +
                         "so they're imported for every race.");

        if (!layers.Any(l => l.Import))
            warnings.Add("No layer in this pack can be imported — see the reasons above.");
    ...
    public ImportResult Register(PreparedImport prepared, bool quiet = false)
    {
        ...
        if (ec != PenumbraApiEc.Success)
        {
            ...
            return new(false, false,
                $"Wrote the mod, but Penumbra couldn't register it ({ec}). Rescan mods in Penumbra.");
        }
        ...
        var tail = prepared.Skipped > 0 ? $" ({prepared.Skipped} layer(s) skipped)" : "";
        if (selectionFailed)
            return new(true, true, $"Imported \"{dirName}\" — {prepared.Imported} layer(s){tail}, but Proteus " +
                $"couldn't pick a body layout for you. Choose one under \"{LayoutGroupName}\" in Penumbra, " +
                "or nothing will paint.");
        if (preview.NeedsAllBodies)
            return new(true, true, $"Imported \"{dirName}\" — {prepared.Imported} layer(s){tail}. The pack has " +
                "nothing painted for a vanilla body, so Proteus set this mod's \"Bodies\" to \"All bodies\" " +
                "(Colors → Advanced) to bake it across — turn that off and it will stop painting.");
        return new(true, false,
            $"Imported \"{dirName}\" — {prepared.Imported} layer(s){tail}. Enabled it and opened it in Penumbra.");
    }
